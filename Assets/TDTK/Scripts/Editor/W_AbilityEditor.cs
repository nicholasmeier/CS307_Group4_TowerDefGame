...
namespace TDTK {
	...
	public class AbilityEditorWindow : TDEditorWindow {
		...
		public static void Init (int prefabID=-1) {
			...
			if(prefabID>=0) window.selectID=AbilityDB.GetPrefabIndex(prefabID);
		}
		...
		protected float DrawGeneralSetting(float startX, float startY, Ability item){
			...
			while(item.stats.cost.Count<RscDB.GetCount()) item.stats.cost.Add(0);
			while(item.stats.cost.Count>RscDB.GetCount()) item.stats.cost.RemoveAt(item.stats.cost.Count-1);
		...
		private int _NewItem(int idx=-1){
			...
			if(idx>=0) item=AbilityDB.GetList()[idx].Clone();
		...
		private void SelectItem(int newID){ 
			selectID=newID;
			if(AbilityDB.GetList().Count<=0) return;
