		...
		public static List<Vector3> Search(NodeTD startN, NodeTD endN, NodeTD[] graph, NodeTD blockN=null, bool smoothing=true, int footprint=-1){
			...
			while(true){
				...
				foreach(NodeTD neighbour in currentNode.neighbourNode){
					...
					if(EnableDiagonal()){
						Vector3 dir=(neighbour.GetPos()-currentNode.GetPos()).normalized;
						if(dir.x*dir.z!=0){
							foreach(NodeTD subN in currentNode.neighbourNode){
								if(subN==neighbour) continue;
								if(Vector3.Distance(subN.GetPos(), neighbour.GetPos())<TowerManager.GetGridSize()*1.1){
									if(subN.IsBlocked()) adjBlocked=true;
								}
							}
						}
					}
					if(adjBlocked) continue;
