...
namespace TDTK {
	...
	public class SpawnEditorWindow : TDEditorWindow {
		...
		private float DrawGeneralSetting(float startX, float startY){
			...
			if(instance.autoStart) instance.startTimer=EditorGUI.FloatField(new Rect(startX+spaceX+height+5, startY, widthS, height), instance.startTimer);
		...
		public float DrawGeneratorParameter(float startX, float startY){
			...
			DrawGenAttribute(startX, cachedY, gen.attPerkRscGainOnCleared, "Perk Rsc Gain:", "", PerkDB.GetRscIcon());		startX+=genAttBlockWidth+10;
			DrawGenAttribute(startX, cachedY, gen.attAbilityRscGainOnCleared, "Ability Rsc Gain:", "", AbilityDB.GetRscIcon());		startX+=genAttBlockWidth+10;
